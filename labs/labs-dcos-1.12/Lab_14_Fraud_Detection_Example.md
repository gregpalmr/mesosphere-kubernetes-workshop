# Lab 14 - Fast Data: Financial Transaction Processing with Apache Flink and Kubernetes

During this example we use Apache Flink and Apache Kafka to setup a high-volume financial transactions pipeline.
Note, this is a derivate of the [Apache Flink Stream Processing](flink/1.11#fast-data-financial-transaction-processing-with-apache-flink) example, where we use Kubernetes to deploy the generator and viewer microservices.
...
A video of this example can be found [here](https://www.youtube.com/watch?v=bwPXNlVHTeI).
...
![Financial transaction processing example architecture](https://github.com/chrisgaun/demos/blob/master/flink-k8s/1.11/img/kafka-flink-arch.png?raw=true)

This example implements a data processing infrastructure that is able to spot money laundering. In the context of money laundering, we  want to detect amounts larger than $10.000 transferred between two accounts, even if that amount is split into many small batches.  See also [US](https://www.fincen.gov/history-anti-money-laundering-laws) and [EU](http://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX%3A32015L0849) legislation and regulations on this topic for more information.
...
The DC/OS services used in the example are as follows:
...
At this point we have all of the required elements installed - Kafka, Flink and the data generator. We are now ready to start the example components.
